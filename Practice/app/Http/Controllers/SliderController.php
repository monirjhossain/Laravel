...
class SliderController extends Controller
{
   ...
   function sliderpost(Request $request){
    ...
    function updateslider($slider_id){
        $slider_name = Slider::find($slider_id)->slider_name;
        $slider_photo = Slider::find($slider_id)->slider_photo;
        return view('admin/slider/update', compact('slider_name', 'slider_id','slider_photo')); 
    }

    function updatesliderpost(Request $request){
        if($request->hasFile('new_slider_photo')){
            // Old photo delete start
        $delete_photo_location = base_path('public/uploads/slider_photos/' . slider::find($request->slider_id)->slider_photo);
